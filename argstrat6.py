...
#print "The possible moves are ",oPossList # initially all the choices are possible
...
def startInteraction(succStrat,cannotAssertP):  
    checkMove=0
    succFlag=1
    oPossList=K_opp
    winnerFlag=0
          ...
          if winnerFlag==1:
             ...
             for key in oPossModels: 
                 if key in oppGuarn:
                    if checkMove==1:
                       checkMove=100
                       print "Computer's move : [",arg,"]"
                       nextMove=1
                       break;
             if checkMove==100:  
                checkMove=0
                for key in propList:
                    if (set(propList[key]) == set(set(succStrat)|set(arg))):#works for unordered lists.
                       dialogueP[key]=set(set(succStrat)|set(arg))        
             ...
             if len(oPossList)==0:
                 winnerFlag=1
startInteraction(succStrat,cannotAssertP)
